...
protected:
	...
	static void ValidateModel(const DefaultWorldCameraModel& m, json expected, int frameNumber);
...
void TestDefaultCameraModel::ValidateModel(const DefaultWorldCameraModel& m, json expected, int frameNumber)
{
...
{
	ValidateModel(reinterpret_cast<DefaultWorldCameraModel&>(*_model), _scenario["frames"][0]["cameraController"], 0);
	for (int i = 0; i < _scenario["frames"].size() - 1; ++i)
	{
		...
		ValidateModel(reinterpret_cast<DefaultWorldCameraModel&>(*_model), framePost["cameraController"], i + 1);
	}
}
