...
SoundLoader::result_type SoundLoader::operator()(BaseLoader<audio::Sound>::FromBufferTag, const pack::AudioBankSample& sample,
                                                 const std::vector<std::vector<uint8_t>>& buffer) const
{
	...
	sound->name = std::filesystem::path(sample.name.data()).filename().string();
	...
	sound->playType = static_cast<audio::PlayType>(sample.loopType);
	sound->buffer = buffer;
	return sound;
}
