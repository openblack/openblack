...
void Audio::Music()
{
	if (ImGui::Button("Play") && _selectedMusicPack != "")
	{
...
void Audio::AudioSettings()
{
	...
	ImGui::NextColumn();
	ImGui::Separator();
	Game::Instance()->GetEntityRegistry().Each<ecs::components::AudioEmitter>(
	...
	Game::Instance()->GetEntityRegistry().Each<ecs::components::AudioEmitter>(
	    [this](entt::entity entity, const AudioEmitter& emitter) {
		    ...
		    auto& sound = Locator::audio::value().GetSound(emitter.soundId);
		    ImGui::SameLine();
		    ImGui::Text("%u", emitter.sourceId);
		    ImGui::NextColumn();
