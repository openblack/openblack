	...
	if (actionSystem.GetMouseDelta() != glm::zero<glm::ivec2>())
	{
		if (actionSystem.Get(input::BindableActionMap::ROTATE_AROUND_MOUSE_ON))
		{
			...
			else if (handPositions[0].has_value() || handPositions[1].has_value())
			{
				...
				const float yaw = delta.x * (glm::two_pi<float>() / windowSize.x);
				float pitch = delta.y * (glm::pi<float>() / windowSize.y);
                                                                                 ...
                                                                                 const Camera& camera)
{
	...
	glm::extractEulerAngleZYX(static_cast<glm::mat4>(orientation), roll, yaw, pitch);
