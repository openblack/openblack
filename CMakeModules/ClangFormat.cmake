...
if (NOT CLANG_FORMAT)
  if (WIN32 AND NOT CMAKE_CROSSCOMPILING)
    ...
    set(CLANG_FORMAT
        "${PROJECT_BINARY_DIR}/externals/clang-format${CLANG_FORMAT_POSTFIX}.exe"
    )
    file(
      DOWNLOAD
    ...
    if (NOT DOWNLOAD_SUCCESS EQUAL 0)
      message(
        WARNING
          "Could not download clang format! Disabling the clang format target"
      )
...
if (CLANG_FORMAT)
  ...
  if (WIN32)
    add_custom_target(
      ...
      COMMAND
        ...
        "Get-ChildItem '${SRCS}/*'  -Include *.cpp,*.h -Recurse | Foreach {&'${CLANG_FORMAT}' -i $_.fullname}"
      COMMAND
        powershell.exe -Command
        "Get-ChildItem '${COMPS}/*' -Include *.cpp,*.h -Recurse | Foreach {&'${CLANG_FORMAT}' -i $_.fullname}"
      COMMAND
        powershell.exe -Command
  ...
  elseif (MINGW)
    add_custom_target(
      clang-format
      COMMAND
        find `cygpath -u ${SRCS}` -iname *.h -o -iname *.cpp | grep -vFf
        ${CMAKE_SOURCE_DIR}/.clang-format-ignore | xargs `cygpath -u
        ${CLANG_FORMAT}` -i
      COMMAND
        find `cygpath -u ${COMPS}` -iname *.h -o -iname *.cpp | grep -vFf
        ${CMAKE_SOURCE_DIR}/.clang-format-ignore | xargs `cygpath -u
        ${CLANG_FORMAT}` -i
      COMMAND
    ...
    add_custom_target(
      clang-format
      COMMAND
        find ${SRCS} -iname *.h -o -iname *.cpp | grep -vFf
        ${CMAKE_SOURCE_DIR}/.clang-format-ignore | xargs ${CLANG_FORMAT} -i
      COMMAND
        find ${COMPS} -iname *.h -o -iname *.cpp | grep -vFf
        ${CMAKE_SOURCE_DIR}/.clang-format-ignore | xargs ${CLANG_FORMAT} -i
      COMMAND
