...
if (NOT TARGET bgfx::shaderc)
  message(
    FATAL_ERROR "Cannot compile shaders without the proper shaderc target"
  )
endif ()

get_target_property(BGFX_INCLUDE_PATH bgfx::bgfx INTERFACE_INCLUDE_DIRECTORIES)
list(GET BGFX_INCLUDE_PATH 0 BGFX_INCLUDE_PATH_1
)# bgfx::bgfx exports include directory twice?
set(BGFX_SHADER_INCLUDE_PATH ${BGFX_INCLUDE_PATH_1}/bgfx)
if (OPENBLACK_USE_BUNDLED_BGFX)
  # non install target builds
  list(APPEND BGFX_SHADER_INCLUDE_PATH ${BGFX_INCLUDE_PATH_1}/../src)
endif ()
...
function (shaderc_parse ARG_OUT)
  ...
  foreach (P ${PLATFORMS})
    if (ARG_${P})
      if (PLATFORM)
        message(
          SEND_ERROR
            "Call to shaderc_parse() cannot have both flags ${PLATFORM} and ${P}."
        )
        return()
      endif ()
      set(PLATFORM "${P}")
  ...
  if ("${PLATFORM}" STREQUAL "")
    message(
      SEND_ERROR
        "Call to shaderc_parse() must have a platform flag: ${PLATFORMS}"
    )
    return()
  ...
  foreach (T ${TYPES})
    if (ARG_${T})
      if (TYPE)
        message(
          SEND_ERROR
            "Call to shaderc_parse() cannot have both flags ${TYPE} and ${T}."
        )
        return()
      endif ()
      set(TYPE "${T}")
  ...
  if ("${TYPE}" STREQUAL "")
    message(
      SEND_ERROR "Call to shaderc_parse() must have a type flag: ${TYPES}"
    )
    return()
